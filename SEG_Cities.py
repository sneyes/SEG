...
class point:
	def __init__(self,_x,_y,_color,_label):
		self.x = _x
		self.y = _y
		self.color = _color
		self.degree = 0
...
def rotatePoint(p, angle, pivot):
	...
	qx += pivot.x
	qy += pivot.y
	return point(qx,qy,p.color,p.label)
...
def rotatePlane(pointSet, angle):
	...
	for p in pointSet:
		...
		newPoints.append(point((qx),(qy),p.color,p.label))
	return newPoints
...
for emanationGrade in range(2,3):
	...
	with open(citiesFile) as csvfile:
		...
		for row in readCSV:
			...
			color=(r,g,b)
			p = point(x*dotSize,y*dotSize,color,lab)
			points.append(p)			
	...
	if emanationGrade == 1:
		...
		for rot in range(0,4):
			...
			for p in rotatedPoints:
				...
				for ps in ySortedPoints:					
					...
					if p.label == "Fortaleza" and ps.label == "Freetown":
						1==1
				...
				j = point(p.x,p_s.y,p.color,p.label)
				isSteiner = False
				if p.x != p_s.x and p.y != p_s.y:
					isSteiner = True				
				if rot != 0:
					p = rotatePointBack(p,point(0,0,p.color,p.label),-1*rot*coneDegree,points)
					p_s = rotatePointBack(p_s,point(0,0,p.color,p.label),-1*rot*coneDegree,points)
					j = rotateJunctionBack(j,-1*rot*coneDegree)
				else:
					p_s = rotatePointBack(p_s,point(0,0,p.color,p.label),0,points)
		...
		for rot in range(8):
			...
			for p in rotatedPoints:
				...
				#CALCULATE JUNCTION POSITION
				j = point(p.x,p_s.y - abs(p_s.x-p.x),p.color,p.label)
				isSteiner = False
				if p.x != p_s.x and p.y != p_s.y:
					isSteiner = True				
				if rot != 0:
